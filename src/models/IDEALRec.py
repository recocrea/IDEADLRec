...
class IDEALRec(CAFM):
    def __init__(self, args):
        super(IDEALRec, self).__init__(args)
    ...
    def calculate_bpr_loss(self, item_seq, pos, neg):
    ...
    def calculate_ce_loss(self, item_seq, pos):
        loss_fct = nn.CrossEntropyLoss()

        seq_emb = self.forward(item_seq).view(-1, self.args.hidden_size)
        labels = pos.view(pos.size(0) * self.args.max_seq_length)
        effective = labels > 0
        logits = torch.matmul(seq_emb[effective], self.item_embeddings.weight.T)
    ...
    def calculate_loss(self, batch):
        ...
        if self.loss_type == 'CE':
            ce_loss = self.calculate_ce_loss(item_seq, pos)
            final_loss = ce_loss + self.args.alpha * self.crm_loss
        else:
            bpr_loss = self.calculate_bpr_loss(item_seq, pos, neg)
